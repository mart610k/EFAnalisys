    ...
    class Program
    {
        ...
        static void AddIngredients()
        {
            using (IngredientContext ctx = new IngredientContext())
            {
                ...
                ctx.AddIngridientBase(new Mixer("Lime Juice")); // This Blocks the key "Lime" from being added thus only one "Lime" can exist
                ctx.AddIngridientBase(new Mixer("Cranberry Juice"));
                ctx.AddIngridientBase(new Accessory("Ice"));
                ctx.AddIngridientBase(new Accessory("Lime")); //Does not get added because "Lime" is taken up as part of "Lime Juice"
            }
        }
